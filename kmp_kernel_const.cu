...
__constant__ int16_t fail_const[MAX_PATTERN_LENGTH + 1];
__constant__ char pattern_const[MAX_PATTERN_LENGTH];
    ...
    int *global_output, int *global_output_cnt, int max_output_cnt)
{
    ...
    while (i < text_end)
    {
        if (get(text, i) == get(pattern_const, j))
        {
            i++;
            j++;
            if (j == pattern_length)
...
__global__ void KMP_search_basic_kernel(
...
//  int16_t *fail)
{
    ...
    //     shared_memory + sizeof(int) * output_cache_size_per_block + 2 * sizeof(int));
    ...
    // int global_output_start = output_cnt;
    // for (
    //     int i = threadIdx.x;
    ...
    int *output, int max_output_cnt, int16_t *fail)
{
    ...
    THROW_IF_ERROR(cudaMemcpyToSymbol(pattern_const, pattern, pattern_size));
    THROW_IF_ERROR(cudaMemcpyToSymbol(fail_const, fail, fail_size));
    THROW_IF_ERROR(cudaMemset(output_cnt_device, 0, sizeof(int)));
    timer_stop();

    // Prepare to launch the kernel.
    // int shared_memory_size =
    //     pattern_size + fail_size + sizeof(int) * output_cache_size_per_block // per block output cache.
    ...
    KMP_search_basic_kernel<<<num_blocks, block_size>>>(
        text_device, text_length,
        // pattern_device,
        pattern_length,
