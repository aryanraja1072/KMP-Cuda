...
static void get_fail(const char *pattern, int pattern_length, int *fail) {
    int candidate = 0;
    fail[0] = -1;
    unsigned char clear_mask = 0x00000011;
...
void char_compress(const char *text, int text_length, char& text_com) {
    // A: 00; T: 01; C:10; G: 11 
    unsigned char A_mask = 0x00000000;
    unsigned char T_mask = 0x00000001;
    unsigned char C_mask = 0x00000010;
    unsigned char G_mask = 0x00000011;
    for (int idx = 0; idx < text_length; idx++) {
        ...
        if (text_tmp == 'A') {
            ...
            text_com[i] |= (T_mask << (2 * j));
        }
        else if (text_tmp == 'C') {
            text_com[i] |= (C_mask << (2 * j));
        }
        else if (text_tmp == 'G') {
            text_com[i] |= (G_mask << (2 * j));
        }
    }
}
